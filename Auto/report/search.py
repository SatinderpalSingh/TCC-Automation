...
def search(request):
	query = request.GET.get('q', '')
    	if query:
        	results = Job.objects.filter(id = query).values('client__client__first_name','client__client__address_1','clientjob__material__name','suspencejob__field__name','id','job_no','date','site','report_type',)
	else:
        	results = []

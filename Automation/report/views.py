...
''''''
#####################################
#pdf generation by reportlab library#
#####################################
def report_pdf(request):
	# Create the HttpResponse object with the appropriate PDF headers.
#	response = HttpResponse(mimetype='application/pdf')
...
def result(request):
	...
	#{'Head':Head, 'organisation':organisation,},
	context_instance=RequestContext(request))
''''''
def material_type(request):
	job =Job.objects.get(id=request.GET['id'])	
#	material = Material.objects.get(name=request.GET['id'])
#	p = head(material = material)
#	p.save()
#	material1 = material('name')
#	material2 = Material.objects.filter(name = material1)
	
#	if material:
#		material1 = Material.objects.filter(name = material)
#		
	return HttpResponseRedirect(reverse('Automation.report.views.chemical_analysis'))
#	else:
